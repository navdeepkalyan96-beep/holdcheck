...
def _login() -> str:
    ...
    r = requests.post(
        LOGIN,
        json={
            "clientcode": os.environ["ANGEL_CLIENT_CODE"],
    ...
    if not body.get("status"):
        raise RuntimeError(body.get("message") or "Angel login failed")
...
def _spot() -> float:
    """Nifty 50 cash token is 99926000 on NSE in the master; fallback 0."""
    try:
        jwt = _jwt()
        r = requests.post(
            LTP,
...
def snapshot(symbol: str = "NIFTY", expiry_iso: str | None = None) -> dict:
    ...
    # quote a window around ATM if we have spot, else first 80
    if spot and legs:
